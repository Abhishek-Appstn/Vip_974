...
const ServicesRenderItem=({item,index,params,isArabic})=>{
    ...
    return(
        <Pressable style={{width:SCREEN_WIDTH*.95,marginVertical:8,alignSelf:'center',height:SCREEN_WIDTH*.28,backgroundColor:Colors.Black_Bg,borderRadius:12,flexDirection:isArabic?'row-reverse':'row',alignItems:'center',paddingHorizontal:15,justifyContent:'space-between'}} onPress={()=>HandleNavigation(item)}>
    ...
    <View style={{marginHorizontal:SCREEN_WIDTH*.03,alignItems:isArabic?'flex-end':null}}>
    ...
    <View>
    <Text style={{color:Colors.Green1,fontSize:24,fontFamily:'Gibson',fontWeight:'600',textAlign:isArabic?'left':'right'}}>{item.price}</Text>
...
<SafeAreaView>
    ...
    <View style={{flexDirection:isArabic?'row-reverse':'row',alignItems:'center',marginTop:55,}}>
       ...
       <Image source={calendar} style={{height:SCREEN_WIDTH*.05,width:SCREEN_WIDTH*.05, resizeMode:'contain'}} />
        ...
        <Text style={{fontFamily:'Gibson-Regular',fontSize:12,color:Colors.White,marginLeft:5,marginVertical:3}}> {params.selectedTime.join(' | ')}</Text>
        </View>
     ...
     <SafeAreaView>
   <FlatList ListFooterComponent={<Text style={{color:Colors.White,alignSelf:'center',fontFamily:'Gibson'}}>--------End---------</Text>} keyExtractor={item=>item.name} data={type=='Beach'?DataConstants.BeachServiceData:DataConstants.DesertServiceData} contentContainerStyle={{marginTop:10,paddingBottom:200}}renderItem={item=><ServicesRenderItem item={item.item} index={item.index} params={params} isArabic={isArabic}/>}/>
   </SafeAreaView>
    </View>
  )
}
