...
const RenderItem=({icon,Address,header})=>{
    return(
<View style={{flex:1,flexDirection:'row',alignItems:'center',marginHorizontal:SCREEN_WIDTH*.02}}>
...
<View style={{marginHorizontal:SCREEN_WIDTH*.02}}>
    ...
    <View style={{flexDirection:'row',justifyContent:'space-between',width:SCREEN_WIDTH*.7}}>
    <Text style={{color:Colors.Green1,maxWidth:SCREEN_WIDTH*.3}}>{Address} Al Arab Mall, Qatar,Uae</Text>
...
const TowingSummary = (props) => {
  ...
  return (
  <ItemLayout buttonTitle={'View Services'} onPress={()=>navigation.navigate('ServicesPage',params)}>
    <View style={{flex:1,backgroundColor:Colors.Black_Bg,paddingHorizontal:SCREEN_WIDTH*.05}}>
       <DrawerHeaderComponent name={params?.subtype}/>
       <View style={{flex:1,alignItems:'center',flexDirection:'row'}}>
        <Image source={Towing} style={{height:SCREEN_WIDTH*.1,resizeMode:'contain'}}/>
      <View>
      <Text style={{color:Colors.White_Text,fontSize:17,textTransform:'uppercase',width:SCREEN_WIDTH*.5}}>Towing Service</Text>
    ...
    <View style={{flex:1.5,paddingTop:SCREEN_HEIGHT*.01,paddingHorizontal:SCREEN_WIDTH*.05}}>
