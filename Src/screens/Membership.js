...
const PointHistory = () => {
  ...
  return (
    <View style={{ backgroundColor: Colors.Black_M, flex: 1 }}>
      <Text style={{ color: Colors.White, fontFamily: 'Gibson', fontSize: 18, fontWeight: '400', padding: SCREEN_WIDTH * .02 }}>Points history</Text>

      <FlatList scrollEventThrottle={16} data={PointHistoryData} contentContainerStyle={{ flexGrow: 1, paddingBottom: SCREEN_HEIGHT * .25 }} renderItem={({ item, index }) => {
        return (
          <View style={[{ flex: 1, flexDirection: 'row', alignSelf: 'center', width: SCREEN_WIDTH * .9, marginVertical: SCREEN_HEIGHT * .01, alignItems: 'baseline', paddingHorizontal: SCREEN_WIDTH * .03, justifyContent: "space-between", paddingVertical: SCREEN_HEIGHT * .01 }, CustomFlexDirection]}>
