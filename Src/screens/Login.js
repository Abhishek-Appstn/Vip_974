...
Helpers({data:MobileNumber,type:'mobilenumber'})?(
        navigation.navigate(name)):(Snackbar.show({text:"Invalid Mobile Number",backgroundColor:'red'}),setError(true)):navigation.navigate(name)
    }
  return (
    <Layout>
      <SafeAreaView
        ...
        <View style={{marginTop: 20, marginBottom: 10}}>
          <CustomTextInput value={MobileNumber} name="Phone Number" type="PhoneNumber"onChangeText={text=>handleTextChange(text)} error={Error} />
        </View>
...
<Pressable style={{alignSelf:'center',bottom:SCREEN_HEIGHT*.05,position:"absolute"}}onPress={()=>HandleNavigation("Drawer")}>
        <Text
          style={{
            fontSize: 12,
            fontFamily: 'Gibson-Regular',
