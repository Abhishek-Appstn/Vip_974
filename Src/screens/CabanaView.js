...
import { Barcode, Call, ChevronLeft, ChevronRight, CompassNorthEast, email, LocationPin } from '../assets/Images'
...
<View style={{marginHorizontal:SCREEN_WIDTH*.02}}>
    ...
    <SafeAreaView>
        ...
        const LowerComponent=({params,props})=>{
    ...
    <Text style={{color:Colors.Green1,fontSize:8,fontFamily:'Gibson',fontWeight:'400',marginTop:SCREEN_HEIGHT*.01}}><Text style={{fontSize:20}}>{params.price}</Text> QAR</Text>
        ...
        <View style={{margin:SCREEN_WIDTH*.05,marginTop:SCREEN_WIDTH*.05}}>
            ...
            <FlatList showsHorizontalScrollIndicator={false} horizontal data={params.features} renderItem={item=><FeatureRenderItem index={item.index} item={item.item}/>}/>
...
<LowerComponent params={params} props={props}/>
