...
const RenderFlatlistItem=({item,index,Active,setActive})=>{
    return(
        <Pressable style={{flexDirection:'row',justifyContent:'space-between',padding:10,marginVertical:10,alignItems:'center'}} onPress={()=>{Active==item.name?setActive(null):setActive(item.name)}}>
            ...
            <View style={{height:SCREEN_WIDTH*.05,width:SCREEN_WIDTH*.05,borderRadius:SCREEN_HEIGHT,borderColor:Colors.Green1,borderWidth:0.5,alignItems:'center',justifyContent:'center',overflow:'hidden'}}>
            <View style={{height:SCREEN_WIDTH*.03,width:SCREEN_WIDTH*.03,borderRadius:SCREEN_HEIGHT*1,backgroundColor:Active===item.name?Colors.Green1:null}}/>
                </View>
            </Pressable>
    )
}
...
<TextInput style={{height:SCREEN_WIDTH*.12,width:SCREEN_WIDTH*.78,color:Colors.White,marginLeft:SCREEN_WIDTH*.02,fontFamily:'Gibson',fontWeight:'400'}}/>
    ...
    <FlatList ListHeaderComponent={ <Text style={{color:Colors.White,fontSize:18,textTransform:'uppercase',marginTop:SCREEN_WIDTH*.05}}>Choose Location</Text>}contentContainerStyle={{height:SCREEN_HEIGHT*.7,paddingTop:SCREEN_WIDTH*.02}} data={places} renderItem={item=><RenderFlatlistItem item={item.item} index={item.index} Active={Active} setActive={setActive}/>}/>
