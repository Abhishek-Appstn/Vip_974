...
const BookingLayout = ({name,children,size,colors,onPress,screenname,innerColor}) => {
    const{SCREEN_HEIGHT,SCREEN_WIDTH}=Constants.SCREEN_DIMENSIONS
    const{Colors}=Constants

  return (
    <View>
       <View style={{backgroundColor:colors?colors:Colors.Black_Bg,height:SCREEN_HEIGHT*.44,width:SCREEN_WIDTH,borderRadius:15,zIndex:9}}>
       <SafeAreaView style={{width:SCREEN_WIDTH,alignSelf:'center',marginTop:SCREEN_WIDTH*.2,overflow:'hidden',backgroundColor:innerColor?innerColor:null}}>
